...
class Twitter {
	...
	async changeWebsiteTo(url) {
		await this.goToPage(ProfilePage)
		let EH = await this.page.waitForXPath(ProfilePage.editProfile);
		await EH.click()
		EH = await this.page.waitForSelector(ProfilePage.editWebsite);
		await EH.click({ clickCount: 3 })
		await EH.type(url)
		EH = await this.page.waitForXPath(ProfilePage.saveProfileEdits);
		await EH.click()
	...
	async login() {
		...
		try {
			...
			let EH = await this.page.waitForSelector(LoginPage.username);
			...
			await this.page.waitForXPath(LoginPage.loginButton).then((EH)=>EH.click());
			...
			logger.log({level: "error", username: ProfilePageObject.url, message: e.message, id: this.flowID})
			throw e;
		}
