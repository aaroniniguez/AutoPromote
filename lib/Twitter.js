...
class Twitter {
	...
	async sendMessageOnDMRequest() {
		...
		let EH = await this.page.$$(UISelectors.dmRequests);
		if(EH.length < 1)
			return
		let newEH = EH[0]
	...
	async saveFollowingCount(DB) {
		...
		let followingCountSelection = UISelectorsObject.numberFollowing(this.credentials.username);
	...
	async saveFollowerCount(DB) {
		await this.goToPage(this.profilePage);
		let followerCountSelection = UISelectorsObject.numberOfFollowers(this.credentials.username);
	...
	async changeWebsiteTo(url) {
		await this.goToPage(this.profilePage)
		let EH = await this.page.waitForXPath(UISelectorsObject.editProfile);
		await EH.click()
		EH = await this.page.waitForSelector(UISelectorsObject.editWebsite);
		await EH.click({ clickCount: 3 })
		await EH.type(url)
		EH = await this.page.waitForXPath(UISelectorsObject.saveProfileEdits);
		await EH.click()
	...
	async getAllFollowButtons() {
		await this.guardInit()
		await this.page.waitForXPath(UISelectorsObject.whoToFollow);
		let results = await this.page.$x(UISelectorsObject.whoToFollow);
		return results
	}
	...
	async followRandomPeople() {
		...
		await this.goToFollowerPage()
		await this.page.waitForXPath(UISelectorsObject.whoToFollow)
		var results = await this.page.$x(UISelectorsObject.whoToFollow);
	...
	async login() {
		try {
			...
			let EH = await this.page.waitForSelector(UISelectorsObject.username);
			await EH.type(this.credentials.username, this.typeDelay);
			await this.page.type(UISelectorsObject.password, this.credentials.password, this.typeDelay);
			...
			logger.log({level: "error", username: this.profilePage, message: e.message, id: this.flowID})
			throw e;
		}
		logger.log({level: "info", username: this.profilePage, message: `Logged in`, id: this.flowID})
	}
	...
	async tweet(data, uploadFile = false) {
		 try {
			...
			logger.log({level: "error", username: this.profilePage, message: e, id: this.flowID});
		}
		logger.log({level: "info", username: this.profilePage, message: `Tweeted ${data}`, id: this.flowID})
		return;
	};
