...
class Twitter {
	...
	async login() {
		try {
			logger.log({level: "info", ...this.credentials.username, message: `Logging in`})
			await this.guardInit()
			...
			logger.log({level: "error", ...this.credentials.username, message: e.message})
			throw e;
		}
	}
	...
	async tweet(data, uploadFile = false) {
		 try {
			...
			if(!this.loggedon)
				await this.login()
			logger.log({level: "info", ...this.credentials.username, message: `Tweeting ${data}`})
			...
			logger.log({level: "error", ...this.credentials.username, message: e});
		}
		return;
	};
