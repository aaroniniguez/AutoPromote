...
class Twitter {
	...
	async login() {
		try {
			logger.log({level: "info", username: this.profilePage, message: `Logging in`, id: this.flowID})
			await this.guardInit()
	...
	async tweet(data, uploadFile = false) {
		 try {
			...
			if(!this.loggedon)
				await this.login()
			logger.log({level: "info", username: this.profilePage, message: `Tweeting ${data}`, id: this.flowID})
