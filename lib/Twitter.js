...
class Twitter {
	...
	async login() {
		try {
			logger.log({level: "info", username: this.profilePage, message: `Logging in`})
			await this.guardInit()
			...
			logger.log({level: "error", username: this.profilePage, message: e.message})
			throw e;
		}
	}
	...
	async tweet(data, uploadFile = false) {
		 try {
			...
			if(!this.loggedon)
				await this.login()
			logger.log({level: "info", username: this.profilePage, message: `Tweeting ${data}`})
			...
			logger.log({level: "error", username: this.profilePage, message: e});
		}
		return;
	};
