...
module.exports = {
	...
	postOnTwitter: async function postOnTwitter(username, password, data, uploadFile = false, randomFollow = false) {
		...
		if(randomFollow) {
			...
			//hmmm try awaiting for non null value of the link...

			await page.waitForXPath("//span[text()='Follow']")
			.then((result) => {
				console.log(typeof result, Object.prototype.toString.call(result));

				result.click(delay = 1000);
			});
			//process.exit();
			var suggestedFollowersLength = await page.evaluate(() => {
				for(let i=0; i < 3; i++) {
					//document.evaluate("//span[text()='Follow']", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click();
				}
			}).catch(function(e) {
				console.log(e);
			});
