...
async function getPNLMessage(pnl){
	var pnlMessages = [];
	return await database.query("select * from stockMessages order by pnl desc").then(rows => {
		var usedPnl;
		...
		return pnlMessages[Math.floor(Math.random()*pnlMessages.length)];
	}).catch((err)=>{
